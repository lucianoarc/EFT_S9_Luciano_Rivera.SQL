...
--creacion de la consulta de clientes por region
CREATE OR REPLACE VIEW RESUMEN_CLIENTES_POR_REGION AS
SELECT
r.nombre_region AS REGION,
COUNT(CASE WHEN MONTHS_BETWEEN(SYSDATE, c.fecha_inscripcion) / 12 >= 20 THEN 1 END) AS CLIENTES_20_ANIOS,
...
r.nombre_region
ORDER BY
CLIENTES_20_ANIOS ASC;
COMMIT;

--SELECT * FROM RESUMEN_CLIENTES_POR_REGION;

--creacion de index
-- Índice para optimizar búsquedas por región
CREATE INDEX IDX_REGION ON cliente (cod_region);

-- Índice para optimizar búsquedas combinadas por región y cliente
CREATE INDEX IDX_CLI_REGION ON cliente (cod_region, numrun);


--consulta informe 2 (alternativa set (Union))
SELECT
TO_CHAR(SYSDATE, 'DD-MM-YYYY') AS FECHA,
tt.cod_tptran_tarjeta AS CODIGO,
tt.nombre_tptran_tarjeta AS DESCRIPCION,
ROUND(AVG(c.valor_cuota)) AS "MONTO PROMEDIO TRANSACCION"
FROM
cuota_transac_tarjeta_cliente c
INNER JOIN transaccion_tarjeta_cliente t ON c.nro_tarjeta = t.nro_tarjeta AND c.nro_transaccion = t.nro_transaccion
INNER JOIN tipo_transaccion_tarjeta tt ON t.cod_tptran_tarjeta = tt.cod_tptran_tarjeta
WHERE
EXTRACT(MONTH FROM c.fecha_venc_cuota) BETWEEN 6 AND 9
GROUP BY
tt.cod_tptran_tarjeta, tt.nombre_tptran_tarjeta
...
INNER JOIN tipo_transaccion_tarjeta tt ON t.cod_tptran_tarjeta = tt.cod_tptran_tarjeta
WHERE
EXTRACT(MONTH FROM c.fecha_venc_cuota) BETWEEN 10 AND 12
GROUP BY
tt.cod_tptran_tarjeta, tt.nombre_tptran_tarjeta
ORDER BY
"MONTO PROMEDIO TRANSACCION" ASC;
COMMIT;

--consulta informe 2 (alternativa subconsulta)
INSERT INTO seleccion_tipo_transaccion (fecha, cod_tipo_transac, nombre_tipo_transac, monto_promedio)
SELECT
TO_CHAR(SYSDATE, 'DD-MM-YYYY') AS FECHA,               -- Formato de fecha
tt.cod_tptran_tarjeta AS COD_TIPO_TRANSAC,             -- Código del tipo de transacción
tt.nombre_tptran_tarjeta AS NOMBRE_TIPO_TRANSAC,       -- Nombre del tipo de transacción
ROUND(AVG(c.valor_cuota)) AS MONTO_PROMEDIO           -- Promedio redondeado
FROM
cuota_transac_tarjeta_cliente c
INNER JOIN transaccion_tarjeta_cliente t ON c.nro_tarjeta = t.nro_tarjeta AND c.nro_transaccion = t.nro_transaccion
INNER JOIN tipo_transaccion_tarjeta tt ON t.cod_tptran_tarjeta = tt.cod_tptran_tarjeta
...
tt.cod_tptran_tarjeta, tt.nombre_tptran_tarjeta
ORDER BY
MONTO_PROMEDIO ASC;  -- Orden ascendente por promedio
COMMIT;

--actualizacion de interes
UPDATE tipo_transaccion_tarjeta tt
SET tt.tasaint_tptran_tarjeta = tt.tasaint_tptran_tarjeta - 1
WHERE EXISTS (
SELECT 1
FROM seleccion_tipo_transaccion st
WHERE st.cod_tipo_transac = tt.cod_tptran_tarjeta
);
--consulta del informe
SELECT 
cod_tptran_tarjeta AS "COD_TPTRAN_TARJETA", 
nombre_tptran_tarjeta AS "NOMBRE_TPTRAN_TARJETA", 
TO_CHAR(tasaint_tptran_tarjeta, '0.00') AS "TASAINT_TPTRAN_TARJETA"
FROM 
tipo_transaccion_tarjeta
WHERE 
cod_tptran_tarjeta IN (
SELECT cod_tipo_transac
FROM seleccion_tipo_transaccion
)
ORDER BY 
cod_tptran_tarjeta ASC;
COMMIT;
/*
1.¿Cuál es el problema que se debe resolver?
El problema consiste en generar un informe que identifique y reporte las transacciones cuyas cuotas vencen entre los meses de junio y diciembre (segundo semestre del año), agrupándolas por tipo de transacción. Además, se debe calcular el promedio de los montos asociados a estas cuotas y ordenar los resultados por monto promedio de forma ascendente. Posteriormente, con base en este informe, se debe actualizar la tasa de interés de los tipos de transacción involucrados, aplicando una rebaja del 1%. La solución debe implementarse utilizando dos enfoques diferentes: uno con subconsulta y otro con el operador SET (UNION).

2.¿Cuál es la información significativa que necesita para resolver el problema?
De la tabla cuota_transac_tarjeta_cliente:
Fechas de vencimiento de las cuotas (fecha_venc_cuota).
Valores de las cuotas (valor_cuota).

De la tabla transaccion_tarjeta_cliente:
Relación entre transacciones y las cuotas (nro_tarjeta, nro_transaccion).
De la tabla tipo_transaccion_tarjeta:

Código del tipo de transacción (cod_tptran_tarjeta).
Nombre del tipo de transacción (nombre_tptran_tarjeta).
Tasa de interés del tipo de transacción (tasaint_tptran_tarjeta).

Información General:
Promedios de los montos de cuotas por tipo de transacción.
Fechas actuales del sistema (SYSDATE) para trabajar con rangos dinámicos.

3.¿Cuál es el propósito de la solución que se requiere?
Reportar Información:
Generar un informe que muestre las transacciones con vencimientos en el segundo semestre, ordenadas por el monto promedio de las cuotas, para analizar y comprender el comportamiento de los clientes.

Optimización de Términos Financieros:
Aplicar una rebaja del 1% en la tasa de interés de los tipos de transacción incluidos en el informe, lo que podría mejorar la experiencia del cliente y fomentar el uso de servicios financieros.
...
Identificar las tablas relacionadas (cuota_transac_tarjeta_cliente, transaccion_tarjeta_cliente, tipo_transaccion_tarjeta) y las columnas clave para la consulta.
Filtrar las transacciones del segundo semestre:
Utilizar EXTRACT(MONTH FROM fecha_venc_cuota) para filtrar cuotas con vencimientos entre junio (6) y diciembre (12).

Calcular el promedio de montos por tipo de transacción:
Agrupar por cod_tptran_tarjeta y nombre_tptran_tarjeta para calcular el promedio de las cuotas (AVG(valor_cuota)).

Insertar los resultados en la tabla SELECCIÓN_TIPO_TRANSACCIÓN:
Crear una consulta con subconsulta que almacene los resultados en la tabla.

Actualizar la tasa de interés:
Usar los códigos de transacción almacenados en SELECCIÓN_TIPO_TRANSACCIÓN para identificar los tipos de transacción y rebajar su tasa de interés en 1%.

Verificar los datos:
Consultar la tabla SELECCIÓN_TIPO_TRANSACCIÓN para validar el reporte generado.
Confirmar la actualización de tasas en la tabla tipo_transaccion_tarjeta.

5.Detalle los pasos, en lenguaje natural, necesarios para construir la alternativa que usa OPERADOR SET.
Definir los datos necesarios:
Identificar las tablas relacionadas (cuota_transac_tarjeta_cliente, transaccion_tarjeta_cliente, tipo_transaccion_tarjeta) y las columnas clave.

Dividir los rangos de meses en dos grupos:
Crear una consulta para filtrar transacciones con vencimientos entre junio (6) y septiembre (9).
Crear otra consulta para filtrar transacciones con vencimientos entre octubre (10) y diciembre (12).

Combinar los resultados con UNION:
Usar el operador SET (UNION) para combinar ambas consultas, asegurando que los datos de los dos rangos estén en el mismo conjunto.

Calcular el promedio de montos por tipo de transacción:
Dentro de cada consulta, agrupar por cod_tptran_tarjeta y nombre_tptran_tarjeta y calcular el promedio (AVG(valor_cuota)).

Ordenar los resultados combinados:
Agregar ORDER BY para ordenar el informe final por el promedio de montos en forma ascendente.

Actualizar la tasa de interés:
Usar los códigos de transacción identificados en la consulta para actualizar la tasa de interés en la tabla tipo_transaccion_tarjeta, aplicando una rebaja del 1%.

Verificar los datos:
Validar el informe generado por la consulta combinada.
Confirmar que las tasas de interés han sido actualizadas correctamente.
*/
